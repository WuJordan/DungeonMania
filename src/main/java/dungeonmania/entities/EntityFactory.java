...
public class EntityFactory {
        ...
        for (int i = player.getX() - radius; i < player.getX() + radius; i++) {
            for (int j = player.getY() - radius; j < player.getY() + radius; j++) {
                ...
                if (map.getEntities(np).stream().anyMatch(e -> e instanceof Enemy))
                    continue;
                if (map.getEntities(np).stream().anyMatch(e -> e instanceof Boulder))
                    continue;
                availablePos.add(np);
            }
        }
    ...
    public Sceptre buildSceptre() {
        int mindcontrolDuration = config.optInt("mind_control_duration", Sceptre.DEFAULT_CONTROL_DURATION);
        return new Sceptre(mindcontrolDuration);
    }

    public MidnightArmour buildMidnightArmour() {
        double armourAttack = config.optInt("midnight_armour_attack", MidnightArmour.DEFAULT_ATTACK);
        double armourDefence = config.optInt("midnight_armour_defence", MidnightArmour.DEFAULT_DEFENCE);
    ...
    private Entity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        case "swamp_tile":
            // int swampDelay = config.optInt("movement_factor", SwampTile.DEFAULT_MOVEMENT);
            int swampDelay = jsonEntity.getInt("movement_factor");
            return new SwampTile(pos, swampDelay);
        case "sun_stone":
