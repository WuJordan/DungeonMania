...
public class GameMap {
    ...
    private void initRegisterMovables() {
        ...
        enemies.forEach(e -> {
            game.register(() -> canMoveFromSwamp(e), Game.AI_MOVEMENT, e.getId());
        });
    }

    private void canMoveFromSwamp(Enemy e) {
        if (e instanceof Mercenary && ((Mercenary) e).isAllied()) {
            e.move(game);
        } else {
            if (!e.stuckInSwamp(game.getMap())) {
                e.move(game);
            }
        }
    }
    ...
    public SwampTile getSwampAtPosition(Position position) {
        List<Entity> allEntitiesAtPos = getEntities(position);
        SwampTile swampTarget = null;

        for (Entity e : allEntitiesAtPos) {
            if (e instanceof SwampTile) {
                swampTarget = (SwampTile) e;
            }
        }
        return swampTarget;
    }

    public boolean isSwampAtPosition(Position position) {
        List<Entity> allEntitiesAtPos = getEntities(position);
